...
3. **Phase 5** ✅ (first shipped in v0.5.0-alpha.1; polished in v0.6.0-alpha.1; operator-hardened in v0.6.1) — Fly.io deployment with `fly.toml` + Fly Volume + readiness probe, `fly.deploy` actuator, deploy-llm-agent role, image-push CI, operator console execute/verify workflow, and repeatable LLM-authored prod deploy proof.
