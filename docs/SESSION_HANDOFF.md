...
- **Previous tagged release:** [`v0.6.7`](https://github.com/jaydenpiao/quorum/releases/tag/v0.6.7) —
  ...
  readiness docs, and strict pip-audit lock refresh for the current
  advisory set. Release workflow run
  [`25413091178`](https://github.com/jaydenpiao/quorum/actions/runs/25413091178)
  succeeded on the signed tag and published SBOM asset
  [`quorum-v0.6.7.spdx.json`](https://github.com/jaydenpiao/quorum/releases/download/v0.6.7/quorum-v0.6.7.spdx.json)
  with digest
  `sha256:a28c51de7249bb717437ae63d73a03b02f2d26b0a6e5fd74dbd8ee907625086e`.
  The signed tag object is
...
- **Current post-v0.6.8 hardening state:** Phase 6 remains blocked
  ...
  gate-readiness evidence. `docs/design/phase-6-entry-plan.md` defines
  the first safe worktree lanes after the gate opens and keeps
...
- **Merge autonomy:** on 2026-05-01 the operator granted durable
  ...
  policy.
- **Post-v0.6.7 status:** reliability PRs #135–#137, dependency
...
- **v0.6 release content:** PR #105 packaged the post-Phase-5
  ...
  CLI security check.
- **Test suite:** 534 passing + 13 integration-gated (excluded from CI
...
4. `CHANGELOG.md` — every feature since bootstrap; `v0.6.8` is the
   current release candidate until its signed tag/proof archive lands.
...
- Phase 6 remains blocked until the documented event-schema stability
  ...
  `QUORUM_RELEASE_TAG=v0.6.8 scripts/check_phase6_gate.sh` before
...
- If Phase 6 is still gated, keep the next increment small and
  ...
  ```bash
  QUORUM_RELEASE_TAG=v0.6.8 scripts/check_console_proof.sh
  QUORUM_RELEASE_TAG=v0.6.8 scripts/check_release_proof_archive.sh
  QUORUM_RELEASE_TAG=v0.6.8 scripts/check_live_release.sh
